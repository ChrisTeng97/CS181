...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # Find closest ghost
        closestGhost = ghostPositions[0]
        for ghost in ghostPositions:
            if self.distancer.getDistance(pacmanPosition, ghost) < self.distancer.getDistance(pacmanPosition, closestGhost):
                closestGhost = ghost
        # Find closest action to closest ghost
        closestAction = legal[0]
        for action in legal:
            if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost) \
                < self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, closestAction), closestGhost):
